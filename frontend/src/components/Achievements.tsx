      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {achievements.map((item) => (
            <div
              key={item.title}
              className="text-center p-8 rounded-2xl bg-white/5 border border-white/10 backdrop-blur card-hover"
            >
              <span className="text-5xl mb-4 block">{item.emoji}</span>
              <div className="text-5xl font-black text-emerald-300 mb-2">
                {item.count}
              </div>
